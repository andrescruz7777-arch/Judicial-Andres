...
# =========================
# EXPORTACIÓN A EXCEL + REPORTERÍA
# =========================
if st.session_state.pagares_data:
    ...
    if "Editado Manualmente" in df.columns:
        ...
        modificados = int(df["Campos Modificados"].astype(str).str.lower().ne("sin cambios").sum())
    else:
        modificados = 0
    ...
    st.markdown("---")
    st.subheader("📊 Resumen general de extracción")
    c1, c2, c3 = st.columns(3)
    c1.metric("🧾 Pagarés procesados", total_pagares)
    c2.metric("✏️ Pagarés modificados", f"{modificados}", f"{porcentaje_modificados:.1f}%")
    if promedio_score is not None:
        c3.metric("✅ Efectividad promedio", f"{promedio_score:.1f}%")
    else:
        c3.metric("✅ Efectividad promedio", "N/A")
    st.markdown("---")
